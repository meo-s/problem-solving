...
public class BJ_1687_행렬_찾기 {
    public static int findMaxArea(final int[][][] dp, final int y, final int x) {
        ...
        while (0 <= y - dy && 0 < dp[y - dy][x][1]) {
            w = Math.min(w, dp[y - dy][x][1]);
    ...
    public static void main(final String[] args) throws IOException {
        ...
        var maxArea = 0;
        final var dp = new int[H][W][2]; // 0: y, 1: x
        for (var y = 0; y < H; ++y) {
            for (var x = 0; x < W; ++x) {
                if (m[y][x] == 0) {
                    dp[y][x][0] = (0 < y ? dp[y - 1][x][0] + 1 : 1);
                    dp[y][x][1] = (0 < x ? dp[y][x - 1][1] + 1 : 1);
                    if (maxArea < dp[y][x][0] * dp[y][x][1]) {
